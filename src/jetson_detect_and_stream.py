...
def get_depth_at_point(depth_frame, x: int, y: int) -> float:
    ...
    # Cast to depth_frame if it's a generic frame after filtering
...
def main() -> None:
    ...
    ap.add_argument("--skip-frames", type=int, default=3, help="Process every Nth frame for detection (1=all frames)")
    args = ap.parse_args()
    ...
    try:
        while True:
            ...
            # Run can detection only on some frames to save processing time
            if frame_count % args.skip_frames == 0:
                annotated_rgb, detections = detect_cans(model, colour, args.confidence)
                last_detections = detections
            else:
                # Reuse last detection results but on current frame
                annotated_rgb = colour.copy()
                detections = last_detections
                
                # Redraw last detections on current frame
                ...
                cv2.putText(annotated_rgb, f"Cans detected: {len(detections)}", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
